...
#define LEFTMENU_WIDTH (IPHONE_WIDTH*0.65)
#define ContentViewX (LEFTMENU_WIDTH-2)
#define LastAlpha  0.3
...
- (instancetype)init{
    if (self = [super init]) {
        ...
        pan.delegate = self;
        [homeVC.view addGestureRecognizer:pan];
...
//    NSLog(@"==move:%f=last=%f",move,_lastX);
    if (target <= 0 && target >= - LEFTMENU_WIDTH) {
        ...
        CGFloat alpha = LastAlpha *(homeX/LEFTMENU_WIDTH);
    ...
    NSLog(@"==centerX:%f",_sideBarVC.view.centerX );
    

    
    //手势结束
    if (rec.state == UIGestureRecognizerStateEnded) {
        if (_homeNav.view.x > LEFTMENU_WIDTH/2 ) {//展开侧边栏
            [UIView beginAnimations:nil context:nil];
            [_sideBarVC.view setX:0];
            [_homeNav.view setX:ContentViewX];
            [UIView commitAnimations];
